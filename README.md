...
| `compare-checksum [--dest <path>]` | Read `{dest}/checksum_sha256.txt` and compare it against what's actually there: reports matched/mismatched/missing/extra counts (with details for anything not matched). Exits `0` unless something listed in the manifest is mismatched or missing; `1` otherwise -- files present on disk but not listed in the manifest ("extra") are reported but don't affect the exit code, since a packaged release folder always has some (`id.txt`, `releaseinfo.json`, cover art, `.m3u`) by design. |
