...
letting it read into the lead-in/lead-out to recover samples a nonzero read
offset would otherwise shift out of range. It's opt-in, not the default, for
a real reason: **on at least one drive (ASUS DRW-24F1ST), it reliably hangs
cd-paranoia forever -- not slow, not erroring, just stuck -- when reading the
disc's last track with a positive offset.** This isn't a rare corner case;
it reproduces every time on that hardware. See root `CLAUDE.md` § Gotchas
for the full investigation.

Overreading only ever matters for the *one* track touching the physical edge
of the disc that your read offset shifts into -- the last track for a
positive offset, the first for a negative one. Middle tracks never touch a
disc boundary, so `--overread` never affects them; `rip`/`pipeline` scope
`--force-overread` to just that one boundary track automatically (no
separate flag needed) and print/log which track it was, or that `--overread`
...
overread and finish with the boundary silence-filled instead of losing the
whole track. Both the stall and which strategy recovered the track are
recorded in the console output and the saved rip log.
