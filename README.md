...
### When to use `--overread`

`--overread` (config: `overreads`) passes cd-paranoia's `--force-overread`,
letting it read into the lead-in/lead-out to recover samples a positive
read offset would otherwise shift out of range. It's opt-in, not the
default, for a real reason: **on at least one drive (ASUS DRW-24F1ST), it
reliably hangs cd-paranoia forever -- not slow, not erroring, just stuck --
when reading the disc's last track with a positive offset.** This isn't a
rare corner case; it reproduces every time on that hardware.

Rules of thumb:

- Only turn it on if your drive's configured `readOffsets` entry is large
  enough that skipping the very edge of the disc would actually lose
  audible samples. `CdParanoiaTrackReader.MaxSafeOffsetSamples` (587
  samples, ~1/75s) is the threshold cd-paranoia itself starts misbehaving
  above; offsets far below that (single digits to a few dozen) have
  essentially nothing to gain from overreading and are not worth the risk.
- Before leaving it on by default for a drive (via the config file's
  `overreads` map), manually verify a full rip completes -- there's no
  automated safety check for this the way `cache-check` exists for cache
  defeat.
- If a rip hangs with `--overread` on, don't assume it'll recover --
  `--stall-timeout` (default 1200s/20 min) will eventually kill and retry
  the stuck read, but if you already suspect this exact cause, dropping
  `--overread` and re-running is faster than waiting it out.
- See root `CLAUDE.md` § Gotchas for the full investigation, including why
  this looks like a cd-paranoia software robustness gap rather than a
  simple drive refusal.
