    ...
    [
        ...
        new HelpSection("Convert", [
            new HelpCommand(["pipeline [options]"], [
                ...
                new HelpOption("--overread", "Pass --force-overread for the disc's boundary track only (last track for a positive read offset, first for negative -- middle tracks are never affected). Also on by default for a drive with a true entry in the config file's overreads map. Known to hang cd-paranoia outright (not just error) on at least one drive; see README's '--overread and --skip-overread-on-stall'"),
                new HelpOption("--skip-overread-on-stall", "If --overread's boundary-track attempt stalls (see --stall-timeout), retry the rest of that track with overread off instead of marking it degraded -- accepts a silence-filled boundary to finish the track"),
                new HelpOption("--no-verify", "Single-pass 'fast'/Burst mode: one cd-paranoia read per track instead of a test+copy pair with a CRC32 compare (AccurateRip's own whole-disc check still runs). Cannot be combined with --max-sector-reads or --stall-timeout"),
                new HelpOption("--retries <n>", "Max test+copy cycles per track before giving up on it (default: 5). 0 means no retries at all"),
                new HelpOption("--max-sector-reads <n>", "Cap on cd-paranoia's own per-sector retries (its --never-skip; default: 12, or the config file's maxSectorReads). 0 means retry a bad sector forever"),
                new HelpOption("--stall-timeout <seconds>", "Kill and retry a single cd-paranoia read if it reports no forward progress for this long (default: 1200, or the config file's stallTimeoutSeconds). 0 disables the timeout. Combines multiplicatively with --retries: worst case per track is roughly stall-timeout x (1 or 2, depending on --no-verify) x retries"),
            ]),
