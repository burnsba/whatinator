    ...
    [
        ...
        new HelpSection("Convert", [
            new HelpCommand(["pipeline [options]"], [
                ...
                new HelpOption("--overread", "Pass --force-overread to cd-paranoia (read into the lead-in/lead-out). Also on by default for a drive with a true entry in the config file's overreads map. Only worth it if your read offset is large enough that skipping the disc edge loses real audio -- known to hang cd-paranoia outright (not just error) on at least one drive when offset is positive and reading the last track; see README's 'When to use --overread'"),
