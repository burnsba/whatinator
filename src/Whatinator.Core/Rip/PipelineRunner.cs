        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!options.SkipFlacPackaging)
        {
            flacResult = await _flacPackager
                .PackageAsync(new FlacPackageOptions(options.ReleaseInfo, rawDir, options.DestinationParentDirectory, discNumber, toc.CatalogNumber, toc, options.DiscIdMatched), cancellationToken)
                .ConfigureAwait(false);
        }
